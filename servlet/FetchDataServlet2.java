...
public class FetchDataServlet2 extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (newsElements.size() > 1) { // 두 번째 뉴스 항목이 존재하는지 확인
                Element secondNewsElement = newsElements.get(1); // 두 번째 뉴스 항목 선택
                Element linkElement = secondNewsElement.select("a").first(); // 링크 추출
                Element imgElement = secondNewsElement.select("img").first(); // 이미지 추출
